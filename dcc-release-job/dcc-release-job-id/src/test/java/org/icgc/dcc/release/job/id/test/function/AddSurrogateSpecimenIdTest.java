...
public class AddSurrogateSpecimenIdTest {
    ...
    public void testCall(){
        ...
        try {
            int index = 99;
            String json = String.format("{\"specimen_id\":\"SP%d\", \"_project_id\":\"project_%d\"}", index, index);

            ObjectNode objectNode = reader.readValue(json);

            ObjectNode newObjectNode = specimen.call(objectNode);

            assertEquals("SP" + index, newObjectNode.get("_specimen_id").textValue() );

            index = 200;
            json = String.format("{\"specimen_id\":\"SP%d\", \"_project_id\":\"project_%d\"}", index, index);

            objectNode = reader.readValue(json);
            newObjectNode = specimen.call(objectNode);
            assertEquals("SP" + (((MockIdClient)IdJobTestSuite.factory.create()).getNext_id_specimen() - 1),  newObjectNode.get("_specimen_id").textValue() );
